...
def create_prompt_with_marathi_gpt_format(messages, bos="<bos>", eos="<eos>", add_bos=True):
    formatted_text = ""
    system_message = ""
    for message in messages:
        if message["role"] == "system":
            ...
            raise ValueError(
                "The template only supports 'system', 'user', and 'assistant' roles. Invalid role: {}.".format(
                    message["role"]
                )
            )
    formatted_text = bos + formatted_text if add_bos else formatted_text
...
def create_prompt_with_alpaca_template(messages, bos="<bos>", eos="<eos>", add_bos=False):
    ...
    for message in messages:
        if message["role"] == "system":
            ...
            # For evaluation, just mark the place for the response
            input_text = ""
        else:
            raise ValueError(
